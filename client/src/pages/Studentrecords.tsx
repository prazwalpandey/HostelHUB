...
const Studentrecords: React.FC = () => {
  ...
  return (
    <div className="adminContainer" style={{ width: "100vw", height: "100vh" }}>
      <AdminSidebar />
      <div className="mainpage flex flex-col" style={{ alignItems: "flex-start", background: "white" }}>
        <main className="dashboard mb-3" style={{ width: "100%", height: "auto", padding: "5%" }}>
          <div className="profile" style={{ display: "flex", flexDirection: "column", justifyContent: "flex-start" }}>
            ...
            <div className="add data flex flex-row" style={{ justifyContent: "space-between", alignItems: "center" }}>
              <p>
                Click on "Select File" and Upload a csv, xlsx or xls to add student records.
              </p>
              <FileUpload />
            </div>
          </div>
          ...
          <div className="profile w-full" style={{ display: "flex", flexDirection: "column", justifyContent: "flex-start" }}>
            ...
            <hr className="border-t border-gray-300 border-width-2" />
            <br />
            <div className="search-by-year" style={{ marginBottom: "20px" }}>
              <input
                type="text"
              ...
              <table className="w-full border-collapse border border-gray-300">
                ...
                <tbody>
                  {filteredStudentRecords.map((record, index) => (
                    <tr key={index}>
                      <td className="border border-gray-300 px-4 py-2 cursor-pointer" onClick={() => handleStudentClick(record)}>
                        {record.name}
                      </td>
                      <td className="border border-gray-300 px-4 py-2 cursor-pointer" onClick={() => handleStudentClick(record)}>
                        {record.email}
          ...
          {showDropdown && selectedStudent && (
            <div className="modal-overlay" style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", backgroundColor: "rgba(0, 0, 0, 0.5)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <div className="modal-content" style={{ display: "flex", flexDirection: "column", backgroundColor: "white", padding: "20px", width: "30%", height: "80%", borderRadius: "8px", boxShadow: "0 0 10px rgba(0, 0, 0, 0.2)", alignItems: "center", justifyContent: "space-around" }}>
                ...
                <div className="buttons" style={{ display: "flex", justifyContent: "space-evenly", width: "100%" }}>
                  <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-4 mr-2" onClick={handleCloseModal}>Close</button>
                  <button className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded mt-4 mr-2" onClick={() => handleEdit(selectedStudent)}>Edit</button>
                  <button className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded mt-4" onClick={() => handleDelete(selectedStudent)}>Delete</button>
                </div>
              </div>
            </div>
          )}
